...
dag = DAG(
    dag_id='wx_user_action_label_table_airflow',
    default_args=args,
...
day_partition = SSHOperator(
    ...
    command=" cd /usr/local/bigdata/jobtaskh0/pythonjob/uos/uid_label/ && spark-submit \
                --num-executors 4 \
                --executor-memory 4G \
                --executor-cores 4 \
                --driver-memory 4G \
                ...
                /usr/local/bigdata/jobtaskh0/pythonjob/uos/uid_label/wx_user_action_label_table.py  \
                day \
                {{ ds_nodash }} ",
    dag=dag
)
