...
if __name__ == "__main__":
    ...
    print("begin to insert the add data to device_increase_day")
    increase_day_sql = """
            select
                        day.stat_date,
                        ...
                        from
                        base.device_shencut_win where  day= '__DAY1__'  and first_active = '__DAY1__' AND app_version is not null
                        group by
                        first_active,
                        app_version
                        ) day 	
                        ...
                        count(devid) as increase
                        from
                        base.device_shencut_mac where   day= '__DAY1__' and first_active = '__DAY1__' AND app_version is not null
                        group by
                        first_active,
                        app_version
                        ) day 
    ...
    if time_type == 'day':
        increase_day_sql = job_day(date=excute_date,
                                   moudle_sql=increase_day_sql)
        insert_into_mysql(sql_query=increase_day_sql, sql_session=sparksession,
                          table="device_increase_day")
    print("begin to add data to device_active_day")
    ...
    active_day_sql = """
                ...
                count(devid) as increase
                from
                base.device_shencut_win where   day= '__DAY1__' AND  last_active = '__DAY1__' and  app_version is not null
                group by
                last_active,
                app_version
                ...
                count(devid) as increase
                from
                base.device_shencut_mac where   day= '__DAY1__' AND last_active = '__DAY1__' and  app_version is not null
                group by
                last_active,
                app_version
                order by last_active desc
    ...
    if time_type == 'day':
        active_day_sql = job_day(date=excute_date,
                                 moudle_sql=active_day_sql)
        insert_into_mysql(sql_query=active_day_sql, sql_session=sparksession,
