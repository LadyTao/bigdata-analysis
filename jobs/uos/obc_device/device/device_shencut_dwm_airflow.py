...
dag = DAG(
    dag_id='device_shencut_dwm_airflow',
    default_args=args,
...
daily = SSHOperator(
    ...
    command='cd /usr/local/bigdata/jobtaskh0/pythonjob/pyspark_template/ && spark-submit \
                ...
                /usr/local/bigdata/jobtaskh0/pythonjob/uos/device/device_shencut_day.py  \
                day \
                {{ ds }} ',
    dag=dag
)
...
weekly = SSHOperator(
    ...
    command='cd /usr/local/bigdata/jobtaskh0/pythonjob/pyspark_template/ && spark-submit \
                ...
                --jars /usr/hdp/3.0.1.0-187/spark2/jars/mysql-connector-java-5.1.47.jar \
                --driver-class-path /usr/hdp/3.0.1.0-187/spark2/jars/mysql-connector-java-5.1.47.jar \
                /usr/local/bigdata/jobtaskh0/pythonjob/uos/device/device_shencut_week.py  \
                week \
                {{ ds }} ',
    dag=dag
)
...
monthly = SSHOperator(
    ...
    command='cd /usr/local/bigdata/jobtaskh0/pythonjob/pyspark_template/ && spark-submit \
                ...
                --jars /usr/hdp/3.0.1.0-187/spark2/jars/mysql-connector-java-5.1.47.jar \
                --driver-class-path /usr/hdp/3.0.1.0-187/spark2/jars/mysql-connector-java-5.1.47.jar \
                /usr/local/bigdata/jobtaskh0/pythonjob/uos/device/device_shencut_month.py  \
                month \
                {{ ds }} ',
    dag=dag
)
