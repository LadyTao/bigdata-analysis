...
if __name__ == "__main__":
    ...
    sparksession = (SparkSession
                    .builder
                    .appName('new_device_shencut_log')
                    .enableHiveSupport()
                    .getOrCreate())
    ...
    device_shencut_win_sql = """
    insert overwrite table base.device_shencut_win partition(day= '__DAY1__') 
    select t1.devid,t1.first_active,
        ...
        day
        from log.cl_shencut_win where logitem='app_launch' and devid is not null  and day ='__DAY1__'
        ) t2
    ...
    if time_type == 'day':
        device_filmora_win_sql = job_day(date=excute_date,
                                       moudle_sql=device_shencut_win_sql)
        print("device_shencut_win_sql:", device_filmora_win_sql)
        print("begin to insert uos_com.device_shencut_win table ")
        excute(sql_query=device_filmora_win_sql, sql_session=sparksession)

    device_shencut_mac_sql = """
    insert overwrite table base.device_shencut_mac partition(day= '__DAY1__') 
    select t1.devid,t1.first_active,
        if(t2.day is not null,t2.day,t1.last_active) as last_active,
        if(t2.product is not null,t2.product,t1.product_id) as product_id,
        if(t2.version is not null,t2.version,t1.app_version) as app_version
        ...
        params['product_id'] as product,
        day
        from log.cl_shencut_mac where logitem='app_launch' and devid is not null and day ='__DAY1__'
        ) t2
        on t1.devid=t2.devid 
        group by t1.devid,t1.first_active,t1.last_active,t1.product_id,t1.app_version,t2.version,t2.product,t2.day
        union all
    ...
    if time_type == 'day':
        device_shencut_mac_sql = job_day(date=excute_date,
                                       moudle_sql=device_shencut_mac_sql)
        print("device_shencut_mac_sql:", device_shencut_mac_sql)
        print("begin to insert uos_com.device_shencut_mac table ")
        excute(sql_query=device_shencut_mac_sql, sql_session=sparksession)
