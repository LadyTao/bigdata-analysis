...
dag = DAG(
    dag_id='device_shencut_first_airflow',
    default_args=args,
...
device_active_first = SSHOperator(
    ...
    command=" cd /usr/local/bigdata/jobtaskh0/pythonjob/pyspark_template/ && spark-submit \
                ...
                /usr/local/bigdata/jobtaskh0/pythonjob/uos/device/device_shencut_active_first.py  \
                day \
                {{ ds_nodash }} ",
    dag=dag
)
...
device_increase_first = SSHOperator(
    ...
    command=" cd /usr/local/bigdata/jobtaskh0/pythonjob/pyspark_template/ && spark-submit \
                ...
                --jars /usr/hdp/3.0.1.0-187/spark2/jars/mysql-connector-java-5.1.47.jar \
                --driver-class-path /usr/hdp/3.0.1.0-187/spark2/jars/mysql-connector-java-5.1.47.jar \
                /usr/local/bigdata/jobtaskh0/pythonjob/uos/device/device_shencut_increase_first.py  \
                day \
                {{ ds_nodash }} ",
    dag=dag
)
