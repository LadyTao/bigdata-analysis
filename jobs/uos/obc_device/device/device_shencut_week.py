...
if __name__ == "__main__":
    ...
    sparksession = (SparkSession
                    .builder
                    .appName('device_increase_add_week')
                    .enableHiveSupport()
                    .getOrCreate())
    ...
    print("begin to add data to device_active_week")
    active_week_sql = """
            select
            t1.*
            from
...
if __name__ == "__main__":
    ...
    (
            ...
            select
            -- fisrt_active as stat_date,
            date_sub('__DAY1__' ,7) as stat_date, -- 周的第一天
            -- concat(substr(date_sub(day,1),0,7),'-01') as stat_month,
            app_version   ,
            count(devid) as increase
            from
            base.device_shencut_win where day= date_sub('__DAY1__',1)  and first_active between date_sub('__DAY1__' ,7) and date_sub('__DAY1__' ,1)  AND app_version is not null
            group by
            date_sub(next_day(date_sub(first_active,1),'MO'),7),
            app_version
            ) week  order by week.stat_date desc
    ...
    (
            ...
            count(devid) as increase
            from
            base.device_shencut_mac where day=  date_sub('__DAY1__',1)  and first_active between date_sub('__DAY1__' ,7) and date_sub('__DAY1__' ,1)   AND app_version is not null
            group by
            date_sub(next_day(date_sub(first_active,1),'MO'),7),
            app_version
            ) week  order by week.stat_date desc
    ...
    active_week_sql = """
            ...
            (
                ...
                count(devid) as increase
                from
                base.device_shencut_win where day= date_sub('__DAY1__',1)  and  last_active between date_sub('__DAY1__' ,7) and date_sub('__DAY1__' ,1) AND app_version is not null
                group by
                date_sub(next_day(date_sub(last_active,1),'MO'),7),
                app_version
                ) week  order by week.stat_date desc
            ...
            (        select
                ...
                count(devid) as increase
                from
                base.device_shencut_mac where day= date_sub('__DAY1__',1)  and  last_active between date_sub('__DAY1__' ,7) and date_sub('__DAY1__' ,1) AND  app_version is not null
                group by
                date_sub(next_day(date_sub(last_active,1),'MO'),7),
                app_version
                ) week  order by week.stat_date desc
