...
dag = DAG(
    dag_id='device_increase_airflow',
    default_args=args,
...
day_partition = SSHOperator(
    ...
    command=" cd /usr/local/bigdata/jobtaskh0/pythonjob/pyspark_template/ && spark-submit \
                ...
                /usr/local/bigdata/jobtaskh0/pythonjob/uos/uid_label/device_increase_add_day.py\
                day \
                {{ ds_nodash }} ",
    dag=dag
)
...
week_partition = SSHOperator(
    ...
    command=" cd /usr/local/bigdata/jobtaskh0/pythonjob/pyspark_template/ && spark-submit \
                ...
                --jars /usr/hdp/3.0.1.0-187/spark2/jars/mysql-connector-java-5.1.47.jar \
                --driver-class-path /usr/hdp/3.0.1.0-187/spark2/jars/mysql-connector-java-5.1.47.jar \
                /usr/local/bigdata/jobtaskh0/pythonjob/uos/uid_label/device_increase_add_week.py \
                week \
                {{ ds_nodash }} ",
    dag=dag
)
...
month_partition = SSHOperator(
    ...
    command=" cd /usr/local/bigdata/jobtaskh0/pythonjob/pyspark_template/ && spark-submit \
                ...
                --jars /usr/hdp/3.0.1.0-187/spark2/jars/mysql-connector-java-5.1.47.jar \
                --driver-class-path /usr/hdp/3.0.1.0-187/spark2/jars/mysql-connector-java-5.1.47.jar \
                /usr/local/bigdata/jobtaskh0/pythonjob/uos/uid_label/device_increase_add_month.py \
                month \
                {{ ds_nodash }} ",
    dag=dag
)
