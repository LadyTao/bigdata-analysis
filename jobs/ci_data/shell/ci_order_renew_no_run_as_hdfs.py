...
cd  /home/ws/airflow/dags

vim ci_order_renew_no_run_as_hdfs.py
...
dag = DAG(
    dag_id='ci_order_renew_no_run_as_hdfs',
    default_args=args,
...
import_ci_hive = BashOperator(
    ...
    bash_command='bash /usr/local/bigdata/jobtaskh0/shelljob/ci_data/import_ci_hive.sh  ', # 在remote机器/tmp/下面创建一个文件
    dag=dag
)
...
import_expire_member_everyday_sh = BashOperator(
    ...
    #command='/home/ws/airflow/order_uuid_analytics/import.sh ', # 在remote机器/tmp/下面创建一个文件
    bash_command='bash /usr/local/bigdata/jobtaskh0/shelljob/ci_data/import_expire_member_everyday.sh  ', # 在remote机器/tmp/下面创建一个文件
    dag=dag
)
...
renew_day_sh = BashOperator(
    ...
    bash_command='bash /usr/local/bigdata/jobtaskh0/shelljob/ci_data/renew_day_test.sh  ',
    dag=dag
)
...
renew_data_to_mysql_sh = BashOperator(
    ...
    bash_command='bash /usr/local/bigdata/jobtaskh0/shelljob/ci_data/renew_data_to_mysql_partition.sh  ',
    dag=dag
)
